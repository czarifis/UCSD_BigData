...
class pcagroupedbyshapefiles(MRJob):
    
    def __init__(self, *args, **kwargs):
        super(pcagroupedbyshapefiles, self).__init__(*args, **kwargs)
        self.data = {}
        
    

    def configure_options(self):
        super(pcagroupedbyshapefiles, self).configure_options()
...
#            sys.stderr.write('\nnp.sum(cov) : '+str(np.sum(cov)))
            ...
            if summation!=0:
                ...
                #sys.stderr.write('\nyielding D')
                yield (str(group)+str(rows),str(D))
