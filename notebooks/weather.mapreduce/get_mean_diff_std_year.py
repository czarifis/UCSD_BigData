        ...
        try:
            ...
            for key in dic_TMIN:
                if dic_TMAX.has_key(key) and dic_PRCP.has_key(key):
                    ...
                    mm=np.divide(np.array(dic_TMAX[key])+np.array(dic_TMIN[key]),2.0)
                    #mean_MEAN.append(np.mean(mm))
                    #mean_PRCP.append(np.mean(np.array(dic_PRCP[key])))
                    std_MEAN.append(np.std(mm))
            if len(validyear)>0:
                yield (station, np.mean(np.array(std_MEAN)))
            #else:
            #    yield (station, _)
        except Exception, e:
